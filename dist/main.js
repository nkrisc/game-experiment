	    ...
	    function updatePlayerBehavior() {
	        ...
	        switch (Array.from(_input.KEY_STATE.arrows.values()).pop()) {
	            case 40:
	                ...
	                if (player.moveDown() && player.y >= _scene.SCENE.VIEW.bottom() - 50 && _scene.SCENE.bottom() !== _scene.SCENE.VIEW.bottom()) {
	                    console.log('SCENE', _scene.SCENE.top(), _scene.SCENE.right(), _scene.SCENE.bottom(), _scene.SCENE.left());
	                    console.log('VIEW', _scene.SCENE.VIEW.top(), _scene.SCENE.VIEW.right(), _scene.SCENE.VIEW.bottom(), _scene.SCENE.VIEW.left());
	                    (0, _scene.offsetY)(-10);
	                }
	                break;
	            case 39:
	                if (player.moveRight() && player.x >= _scene.SCENE.VIEW.right() - 50 && _scene.SCENE.right() !== _scene.SCENE.VIEW.right()) (0, _scene.offsetX)(-10);
	                break;
	            case 38:
	                if (player.moveUp() && player.y <= _scene.SCENE.VIEW.top() + 50 && _scene.SCENE.top() !== _scene.SCENE.VIEW.top()) (0, _scene.offsetY)(10);
	                break;
	            case 37:
	                if (player.moveLeft() && player.x <= _scene.SCENE.VIEW.left() + 50 && _scene.SCENE.left() !== _scene.SCENE.VIEW.left()) (0, _scene.offsetX)(10);
	                break;
	        }
